...
import (
	...
	"github.com/DamirLuketic/digihey_backend/db/data"
...
func createDB() {
	db, err := sql.Open("mysql", "digihey:digihey@tcp(db:3306)/")
	if err != nil {
	...
	for a := retry.Start(connectRetryStrategy(), nil); a.Next(); {
		_, err = db.Exec("DROP DATABASE IF EXISTS digihey")
		if err == nil {
			break
		} else {
	...
	_, err = db.Exec("CREATE DATABASE digihey")
	if err != nil {
		log.Fatalf(err.Error())
	} else {
